...
func init() {
	api.Register("POST", "aggregate/search", aggregateSearch)
	api.Register("POST", "aggregate/count", aggregateCount)
}

func aggregateSearch(ctx *gin.Context) {
...
func aggregateCount(ctx *gin.Context) {
	...
	ret, err := db.Count(Bucket, filter)
	if err != nil {
		curd.Error(ctx, err)
		return
	}
